...
<nav id="desktop-nav">
    ...
    <section id="ragging-complaints">
        ...
        <div class="title-container">
            <h1>Medical Application Tracking</h1>
        ...
        <div class="data-container">
            <?php
                if(isset($_SESSION['Senrol'])) {
                    $enrol = $_SESSION['Senrol'];
                    $sql_query_medical = "SELECT Id, Reason, SubmissionDate FROM medical WHERE enrol = '" . $enrol . "'";
                    $result_medical = mysqli_query($conn, $sql_query_medical);
                    ...
                    while ($row_medical = mysqli_fetch_assoc($result_medical)) {
                        echo '<tr>';
                        echo '<td>' . $row_medical['Id'] . '</td>';
                        echo '<td>' . $row_medical['Reason'] . '</td>';
                        echo '<td>' . $row_medical['SubmissionDate'] . '</td>';
                        echo '<td><a style="color: darkred" href="?appid=' . $row_medical['Id'] . '">View Details</a></td>';
                        echo '</tr>';
                    }
                    echo '</table>';
                }
                if(isset($_GET['appid'])) {
                    $appid = $_GET['appid'];
                    $sql_query_details = "SELECT * FROM medical WHERE id = '" . $appid . "'";
                    ...
                    while ($row_details = mysqli_fetch_assoc($result_details)) {
                        ...
                        echo '<p class="card-heading">Medical Information</p>';
                        echo '<div class="medical-logo"><img src="./assets/icon.png" alt="Medical Logo"></div>';
                        echo '<p><strong>Application Id :</strong> ' . $row_details['Id'] . '</p>';
                        ...
                        echo '<p><strong>Reason :</strong> ' . $row_details['Reason'] . '</p>';
                        echo '<p><strong>Starting Date :</strong> ' . $row_details['Sdate'] . '</p>'; 
                        echo '<p><strong>Ending Date :</strong> ' . $row_details['Edate'] . '</p>'; 
                        echo '<p><strong>Submission Date :</strong> ' . $row_details['SubmissionDate'] . '</p>'; 
                        $status = $row_details['Status'];
                        $color = ($status == 0) ? 'greenyellow' : 'red';
                        $text = ($status == 0) ? 'Approved' : 'Pending';
